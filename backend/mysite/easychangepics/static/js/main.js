...
// const filterOptions = document.querySelectorAll(".wrapper__sidebar-left__filter-content input");
    ...
    document.getElementById('uploadImage').addEventListener('change', function (e) {
        ...
        if (parts.length > 1) {
            ...
            if (dataPairs[0] === "tgAuthResult") {
                var decodedData = decodeURIComponent(dataPairs[1]);
        ...
        sendBtn.addEventListener('click', function() {
            // Сохранение изображения в localStorage перед авторизацией
            testData = fabricCanvas.toDataURL('image/jpeg');
            localStorage.setItem('unsentImage', testData);
            localStorage.setItem('authData', JSON.stringify(data));
        
            // Проверьте, авторизован ли пользователь
            fetch('/accounts/is_authorized/')
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                if (data.is_authorized) {
                    ...
                    window.location.href = `/accounts/telegram/login/?process=login&next=${redirectUrl}`;
                }
            })
            .catch(error => {
        ...
        function send(){
            // Восстановление изображения из localStorage после авторизации
            testData = localStorage.getItem('unsentImage');
            data = JSON.parse(localStorage.getItem('authData'));
            
            const formData = new FormData();
            formData.append('image', testData); // Добавление данных изображения в FormData
            ...
            .then(response => {
                if (response.ok) {
                    ...
                    localStorage.removeItem('unsentImage');
    ...
    //     // Создание объекта FormData для отправки на сервер
    //     const formData = new FormData();
    //     formData.append('image', dataURL); // Добавление изображения
    //     formData.append('data', JSON.stringify({/* ваши данные здесь */})); // Добавление остальных данных
